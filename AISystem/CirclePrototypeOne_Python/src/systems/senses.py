...
def senseSight(coordinator: ECSCoordinator, terrain: Terrain):
    for entity_id in coordinator.getEntitiesWithComponent(constants.SIGHT_COMPONENT):
        ...
        offset: Point3D = Point3D(math.cos(angle), math.sin(angle), 0) * sight_range.distance * sight_range.offset_factor
        brain_component.entities = [EntityTarget(position, entity_id_iter) for position, entity_id_iter in terrain.entities.query(position + offset - Point3D.fromUniform(sight_range.distance), position + offset + Point3D.fromUniform(sight_range.distance)) if entity_id_iter != entity_id]
