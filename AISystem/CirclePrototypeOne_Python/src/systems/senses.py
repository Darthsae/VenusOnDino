...
def senseSight(coordinator: ECSCoordinator, terrain: Terrain):
    for entity_id in coordinator.getEntitiesWithComponent(constants.SIGHT_COMPONENT):
        ...
        #print(entity_id)
        brain_component.entities = [EntityTarget(position, entity_id_iter) for position, entity_id_iter in terrain.entities.query(position - Point3D.fromUniform(sight_range.distance), position + Point3D.fromUniform(sight_range.distance)) if entity_id_iter != entity_id]
